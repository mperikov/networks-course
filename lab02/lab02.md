...
3.  Обновить существующий продукт
    ![](images/image-4.png)
    ![](images/image-5.png)
4.  Получить список всех продуктов
    ![](images/image-6.png)
...
Последний пакет "начинает" проходить через $(P - 1) \dfrac{L}{R}$ и проходит за $N \dfrac{L}{R}$. Итого $(N + P - 1) \dfrac{L}{R}$.
...
#### Решение
Пусть $L$ - размер пакета, тогда количество пакетов $P = \dfrac{5мб}{L}$. Поскольку $R_1$ самая маленькая, основная задержка будет на первом канале. Последний пакет "начинает" проходть через $(P - 1) \dfrac{L}{R_1}$ и проходит за $\dfrac{L}{R_1} + \dfrac{L}{R_2} + \dfrac{L}{R_3}$.  
  
Итого $P \dfrac{L}{R_1} + \dfrac{L}{R_2} + \dfrac{L}{R_3} \approx \dfrac{5мб}{R_1} = 204.8 с$.  
  
Видна линейная зависимость времени от размера пакета $L$.
...
$P = \dfrac{X}{S}$.  
Время передачи:  
$(3 + P - 1) \dfrac{L}{R} = (\dfrac{X}{S} + 2) \dfrac{80 + S}{R}$  

$f(S)  = X + \dfrac {80X}{S} + 2S + 160$  

$f'(S) = - \dfrac {80X}{S^2} + 2$

$f'(S) = 0$ при $S = \sqrt{40X}$

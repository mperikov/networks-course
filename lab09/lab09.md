...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - ICMP - протокол сетевого уровня, а не транспортного. Он используется для обмена информацией между хостами, а не процессами.
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   ...
   - Контрольная сумма, идентификатор, порядковый номер, data
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Размер данных (data) 64 байта, у ping-запросов 32 байта
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Дополнительно содержатся IP-заголовок и ICMP-данные исходного ICMP-пакета 
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Это обычные ICMP-ответы с типом 0
   - Этим пакетам хватает TTL, чтобы дойти до конечного хоста, поэтому ошибки не возникает и хост отправляет обычный ICMP-ответ типа 0.
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Канал между spb-81-211-89-156.liteline.ru 81.211.89.156 и MX01.HongKong.gldn.net 79.104.235.120. Задержка более 100ms
   - Санкт-Петербург и Гонконг
...
#### Демонстрация работы
lab09/freeport  
Перебираем порты в диапозоне и пробуем установить TCP-соединение с каждым, если получилось - порт доступен.
```bash
$ go run . -ip 8.8.8.8 -from 50 -to 60
```
![](images/image-2.png)

...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Time to Live, Identification (и Header Checksum) 
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Source Address, Destination Address, Version, Header Length, Protocol
      - Flags, Fragment Offset (если нет фрагментации), dsfield
      - Time to Live, Identification (и Header Checksum) 
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Identification меняется, TTL всегда 64
...
lab10/main.go
```bash
$ go run . -host 8.8.8.8 -count 10
```

```checksum``` из lab08 не подошла для ICMP, новая [реализация](./main.go#L279-L297).

...
#### Решение
Пусть $X$ - средняя пропускная способность в сегментах (MSS) за RTT.  
$X = \overline{W} = \frac{\frac{W}{2} + W}{2} = \frac{3}{4}W$  
$T = \Delta W = \frac{W}{2} = \frac{2}{3}X$
...
#### Решение
Время на установление TCP-соединения считаем как $2RTT$.  
В фазе медленного старта TCP размер сегмента будет экспоненциально увеличиваться:  
$1S + 2S + 4S + 8S = 15S$  
Объект разобъется на 4 блока.  

1. $RTT > \dfrac{S}{R}$ и $RTT < \dfrac{3S}{R}$  
    - блок из 2S начинает отправляться через  
    $$\dfrac{S}{R} + RTT$$  
    - блок из 4S начинает отправляться через  
    $$\dfrac{S}{R}+RTT+\dfrac{2S}{R}+(RTT-\dfrac{S}{R})$$  
    - блок из 8S начинает отправляться через  
    $$\dfrac{S}{R}+RTT+\dfrac{2S}{R}+(RTT-\dfrac{S}{R})+\dfrac{4S}{R}$$  
    - Итого  
    $$\dfrac{14S}{R} + 2RTT$$  
2. $𝑅𝑇𝑇 > \dfrac{3𝑆}{𝑅}$
    - блок из 2S начинает отправляться через  
    ...
    $$\dfrac{S}{R}+RTT+\dfrac{2S}{R}+(RTT-\dfrac{S}{R})$$  
    - блок из 8S начинает отправляться через  
    $$\dfrac{S}{R}+RTT+\dfrac{2S}{R}+(RTT-\dfrac{S}{R})+\dfrac{4S}{R}+(RTT-\dfrac{3S}{R})$$  
    - Итого  
    $$\dfrac{11S}{R} + 3RTT$$ 
3. $𝑅𝑇𝑇 < \dfrac{𝑆}{𝑅}$
    - блок из 2S начинает отправляться через  
    $$\dfrac{S}{R} + RTT$$  
    - блок из 4S начинает отправляться через  
    $$\dfrac{S}{R}+RTT+\dfrac{2S}{R}$$  
    - блок из 8S начинает отправляться через  
    $$\dfrac{S}{R}+RTT+\dfrac{2S}{R}+\dfrac{4S}{R}$$  
    - Итого  
    $$\dfrac{15S}{R} + RTT$$

Везде добавляем $2RTT$ за установление TCP-соединения.
...
#### Решение
Пусть размер окна перегрузки увеличивается с $\frac{W}{2}$ до $W$ за время $C = const$.  
Пусть $K$ - число полученных ACK-пакетов за это время.  
Тогда $(1 + a)^K = 2$ и $K = \log_{1 + a}{2}$.  
Пусть за время $C$ было отправлено $C*\overline{W}$ пакетов, и $\overline{W} \propto W$.  
Тогда $L = \frac{C*\overline{W} - K}{C}$, т.е. $L \propto W$.

...
1. Выполните nslookup, чтобы получить IP-адрес какого-либо веб-сервера в Азии  
   Получен IP-адрес веб-сервера: 210.152.243.234   
   ![](images/image-10.png)  
...
4. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ...
   Тип A (браузер также отправил DNS запрос типа HTTPS)  
   Ответов нет  
   ![](images/image-5.png)
5. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в
   каждом?  
   2 ответа. В каждом информация про хост, в том числе IP-адрес.  
   ![](images/image-6.png)
...
7. Веб-страница содержит изображения. Выполняет ли хост новые запросы DNS перед
   ...
   Делается запрос на static.ietf.org, затем начинается загрузка.  
   ![](images/image-8.png)
   ![](images/image-9.png)
...
3. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ли в запросе какие-нибудь «ответы»?  
   Запрашивается запись типа AAAA (IP6)  
   Ответов нет
   ![](images/image-17.png)

4. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в каждом?  
   Получен один ответ, в нем информация о том, что `www.spbu.ru` — алиас, а каноническое имя — `spbu.ru`  
   ![](images/image-18.png)
...
3. Проанализируйте ответное сообщение DNS. Имена каких DNS-серверов университета в
   ...
   ns.pu.ru  
   ns2.pu.ru  
   Для ns7.spbu.ru есть адрес 185.44.15.195  
...
1. На какой IP-адрес отправлен DNS-запрос? Совпадает ли он с адресом локального DNS-сервера, установленного по умолчанию? 
   ...
   На 195.70.196.210, не совпадает с локальным DNS, принадлежит spbu.ru  
   ![](images/image-21.png)
...
1. Что такое база данных whois?
   Whois — это общедоступная база регистрационных данных о доменах и IP-адресах: владелец/регистратор, даты регистрации и истечения, а также технические параметры, включая NS-записи (DNS-серверы домена).
2. Используя различные сервисы whois в Интернете, получите имена любых двух DNS-серверов. 
   ...
   - Сервис [ICANN Lookup](https://lookup.icann.org/)
   - Сервис [who.is](https://who.is/)
   ![](images/image-25.png)
   ![](images/image-26.png)
3. Используйте команду nslookup на локальном хосте, чтобы послать запросы трем конкретным
   ...
   - Пример выполнения:
     - `nslookup www.spbu.ru` (локальный DNS по умолчанию)
     - `nslookup www.spbu.ru ns.pu.ru`

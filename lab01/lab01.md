...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - ru/en
   - IP адрес, версия браузера и ОС, принимаемые форматы файлов
...
![screen2](data/image-2.png)
![screen3](data/image-3.png)
![screen4](data/image-4.png)
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да
   - Status Code: 200, File Data: 371 bytes
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Да
   - Дата Last Modified полученного из первого запроса файла
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - были загружены параллельно, т.к. второй GET запрос (No 117) был сделан до получения ответа (No 165) на первый GET (No 107)

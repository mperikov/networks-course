...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Send e-mail with image attachment via SMTP using sockets."
    )
    parser.add_argument("recipient", help="Recipient e-mail address")
    parser.add_argument("image_path", help="Path to image file to attach")
    parser.add_argument(
        "--subject",
        default="Lab05 SMTP socket client with image",
        help="Message subject",
    )
    parser.add_argument(
        "--body",
        default="Message with image attachment sent via SMTP over sockets.",
...
def encode_base64_lines(data: bytes, line_len: int = 76) -> list[str]:
    encoded = base64.b64encode(data).decode("ascii")
    return [encoded[i:i + line_len] for i in range(0, len(encoded), line_len)]


def build_multipart_message(
    sender: str, recipient: str, subject: str, body: str, image_path: str
) -> str:
    file_path = Path(image_path)
    if not file_path.exists():
        raise FileNotFoundError(f"Attachment file not found: {image_path}")

    data = file_path.read_bytes()
    encoded_lines = encode_base64_lines(data)
    filename = file_path.name
    boundary = "LAB05_BOUNDARY_42"
    mime_type = mimetypes.guess_type(filename)[0] or "application/octet-stream"

    body_lines = body.splitlines() or [""]
    escaped_body_lines = ["." + line if line.startswith(".") else line for line in body_lines]
...
def send_email_via_smtp_socket(args: argparse.Namespace) -> None:
    ...
    with raw_sock:
        ...
        with tls_sock:
            ...
            send_smtp_command(tls_sock, tls_file, f"MAIL FROM:<{sender}>", {250})
            send_smtp_command(
                tls_sock,
                tls_file,
            ...
            message = build_multipart_message(
                sender, args.recipient, args.subject, args.body, args.image_path
            )
...
def main() -> None:
    ...
    print(f"Message with image sent successfully to {args.recipient}.")
